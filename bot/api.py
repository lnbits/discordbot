...
class LnbitsAPI:
    def __init__(self, *, admin_key: str, http: AsyncClient, lnbits_url: str, **options):
    ...
    async def get_lnbits_user(self, discord_user: DiscordUser):
        users = await self.request('GET',
                                   '/users',
                                   self.admin_key,
                                   extension='usermanager',
                                   params={
                                       'extra.discord_id': str(discord_user.id)
                                   })
        if users:
            user = users[0]
            if user["extra"].get("discord_avatar_url") != discord_user.display_avatar.url:
                try:
    ...
    async def get_user_wallet(self, discord_user: DiscordUser) -> Optional[Wallet]:
        ...
        if not wallet:
            ...
            if user:
                wallets = await self.request('GET',
                                             f'/wallets/{user["id"]}',
                                             self.admin_key,
                                             extension='usermanager')
    ...
    async def get_or_create_wallet(self, user: DiscordUser) -> Wallet:
        ...
        if not wallet:
            user = await self.request('POST', '/users',
                                      self.admin_key,
                                      extension='usermanager',
                                      json=dict(
                                          user_name=user.name,
    ...
    async def request(self,
        ...
        response = await self.lnbits_http.request(
            method,
            url=self.lnbits_url + (extension + '/' if extension else '') + 'api/v1' + path,
            **kwargs
        )
    ...
    async def send_payment(self,
        ...
        invoice = await self.request('POST', '/payments',
        ...
        await self.request('POST', '/payments',
